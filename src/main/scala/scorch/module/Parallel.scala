...
case class Parallel(module: Module, parallelism: Int) extends Module {
  override def forward(x: Variable): Variable =
    ParallelFunction(x, module, parallelism).forward()
}

case class ParallelFunction(x: Variable, module: Module, parallelism: Int)
  ...
  override def backward(gradOutput: Variable): Unit = {
    ...
    val gradient = Variable(ns.concatenate(xs.map(_.grad.data).seq, 0))
    x.backward(gradient)
  }
