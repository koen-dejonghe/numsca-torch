...
object Function {
  def relu(x: Variable)(implicit r: Region): Variable = LeakyRelu(x).forward()
  def leakyRelu(x: Variable, negVal: Double = 0.0)(implicit r: Region): Variable =
    LeakyRelu(x, negVal).forward()
  def logSoftMax(x: Variable, dim: Option[Long] = None)(implicit r: Region): Variable =
  ...
  def mseLoss(x: Variable,
              ...
              reduce: Boolean = true)(implicit r: Region): Variable =
